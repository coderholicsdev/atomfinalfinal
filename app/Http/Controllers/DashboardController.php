...
class DashboardController extends Controller
{
    public function index(){
        ...
        //property
        $responseProp = Http::get("https://api.rentsmallsmall.com/api/property-api");
        $responseProp->body();
        $properties = $responseProp->json();
        
        //landlord
        $responseLandlord = Http::get("https://api.rentsmallsmall.com/api/landlord-api");
        $responseLandlord->body();
        $landlords = $responseLandlord->json();

        //Verifications
        $responseVerifications = Http::get("https://api.rentsmallsmall.com/api/verification-api");
        $responseVerifications->body();
        $verifications = $responseVerifications->json();


        //Transactions
        $responseTransactions = Http::get("https://api.rentsmallsmall.com/api/transaction-api");
        $responseTransactions->body();
        $transactions = $responseTransactions->json();

        //Inspections
        $responseInspections = Http::get("https://api.rentsmallsmall.com/api/inspection-api");
        $responseInspections->body();
        $inspections = $responseInspections->json();

        //bookings === real tenants
        $responseBookingsTenants = Http::get("https://api.rentsmallsmall.com/api/booking-distinct-count-api");
